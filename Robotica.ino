...
//declaracion de angulos
  ...
  c5=sen(q1)*o.x-cos(q1)*o.y;      //que es n y o??
  s5=sen(q1)*n.x-cos(q1)*n.y;
  q5=atan2(s5,c5);
  if(cos(q1)==0){
    q234=atan2(a.z,a.y/sen(q1));
    p=punto_intermedio.y/sen(q1);
  }
  else{
  ...
  B=punto_intermedio.z-l1-la*sen(q234)-l5*sen(q234);
  q3=+arcos((A^2+B^2-l2^2-l3^2)/(2*l2*l3));

  c2=(A*(l2+l3*cos(q3))+B*sen(q3)*l3)/((l2+l3*cos(q3))^2+(sen(q3)*l3)^2);
  s2=(B(l2+l3*cos(q3))-A*l3*sen(q3))/((l2+l3*cos(q3))^2+(sen(q3)*l3)^2);
  q2=atan2(s2/c2);
