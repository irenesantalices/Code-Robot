...
double giro_p1[N_MOTORES] {1.58,-6.18,3.42,-0.92,0.05};
...
if(evaluar_FP()==true){
  ...
  for(i=0;i<N_MOTORES;i++){
    motores[i].q_r = encoders[i]->read()*motores[i].resolucion_encoder;
  }
}
...
void ciclo_trabajo(){
  ...
  const double distancia_seguridad = 0.15;
  //programa
...
void realimentacion(Encoder E, motor& M, int n_motor){
  ...
  while(abs(M.q_r)>2*M_PI){
    if(M.q_r<0) M.q_r+=2*M_PI;
    if(M.q_r>0) M.q_r-=2*M_PI;
  }
  //Excepcion para el gripper
  if(n_motor == 5){
    M.q_r = abs(M.q_r);
  }
  M.q_err = M.q_d - M.q_r;
  ...
  while(true){
    if(evaluar_FP()==true){
      for(int i = 0; i<N_MOTORES; i++){
        q = encoders[i]->read()*motores[i].resolucion_encoder;
        while(abs(q)>2*M_PI){
          if(q<0) motores[i].q_r+=2*M_PI;
          if(q>0) motores[i].q_r-=2*M_PI;
          }
        Serial.print("q");
        Serial.print(i+1);
